...
class CopilotUsageAnalyzer {
     ...
     * Allows users to link directly to specific tabs
     * Examples: #usage-dashboard, #quota-dashboard, #comparison-dashboard
     */
    handleDeepLink() {
        const hash = window.location.hash.substring(1); // Remove the # character
        
        // Valid tab IDs
        const validTabs = ['usage-dashboard', 'quota-dashboard', 'comparison-dashboard'];
        
        if (hash && validTabs.includes(hash)) {
            this.switchTab(hash);
        } else if (!hash) {
            // Default to usage dashboard if no hash
            this.switchTab('usage-dashboard');
        }
    }
